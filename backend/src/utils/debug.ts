 ...
 * API 专用调试日志
 */
export function debugApi(message: string, data: any, options: DebugLogOptions = {}) {
  if (!DEBUG_MODE && !options.enabled) {
    return;
  }
  
  const prefix = options.prefix || '🌐';
  console.log(`${prefix} [API DEBUG] ${message}:`, data);
}

/**
 * 数据库专用调试日志
 */
export function debugDb(message: string, data: any, options: DebugLogOptions = {}) {
  if (!DEBUG_MODE && !options.enabled) {
    return;
  }
  
  const prefix = options.prefix || '💾';
  console.log(`${prefix} [DB DEBUG] ${message}:`, data);
}
